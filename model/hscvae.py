...
class ConditionalVAE(object):
    ...
    def _create_network(self, latent, ini=False):
        ...
        ### HSVAE
        self.z_mean = self.z_mean / tf.reshape(tf.norm((self.z_mean), axis=-1), [-1,1])
        self.q_z = VonMisesFisher(self.z_mean, 100.0*np.ones("n_z"))
        ...
        with tf.name_scope('loss'):
            ...
            ## S-VAE
            self.latent_loss = tf.reduce_mean( self.q_z.kl_divergence(HypersphericalUniform(latent-1)) )
